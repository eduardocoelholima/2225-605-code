...
public class MatrixParallelMultiply extends Thread {
    ...
    MatrixParallelMultiply(int row, int col) { this.row = row; this.col = col; }
    ...
    public static void main(String[] args) {
        ...
        for (int row = 0; row < matrix1.length; row++)
            for (int col = 0; col < matrix2[0].length; col++) {
                MatrixParallelMultiply task = new MatrixParallelMultiply(row, col);
                threads.add(task);
